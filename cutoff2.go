...
func Cutoff2[A, B any](epsilon Incr[A], input Incr[B], fn Cutoff2Func[A, B]) Incr[B] {
	return Cutoff2Context[A, B](epsilon, input, func(_ context.Context, epsilon A, oldv, newv B) (bool, error) {
...
func Cutoff2Context[A, B any](epsilon Incr[A], input Incr[B], fn Cutoff2ContextFunc[A, B]) Cutoff2Incr[A, B] {
	o := &cutoff2Incr[A, B]{
		n:  NewNode(),
		fn: fn,
		e:  epsilon,
